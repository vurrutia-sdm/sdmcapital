...
| 2026-08-09 | Sistema de diseño: dos arreglos críticos | **ZONA COMPARTIDA: `tailwind.config.js` y `globals.css`.** (1) `muted` valía `#7a8a96` en el config y `#5F7183` en globals — el primero da 3,56:1 y no cumple AA. Se alinea al que cumple. (2) Los cinco `.btn-*` pasan a tener hover por color en vez de por opacidad, más `:active` y `:disabled` unificados | En curso |
| — | Sofía / chatbot | — | — |
