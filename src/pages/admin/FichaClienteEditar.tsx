    ...
    <div style={{ minHeight: '100vh', background: '#f5f7fa', fontFamily: 'inherit' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #dce4ec', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <Link to={`/admin/ficha-cliente/${clienteId}/ficha/${fichaId}`}
            style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#7a8fa6', textDecoration: 'none', fontSize: 13 }}>
            <ArrowLeft size={16} /> Cancelar
          </Link>
          <span style={{ color: '#dce4ec' }}>|</span>
          <span style={{ fontSize: 16, fontWeight: 600, color: '#0d2240' }}>Editar ficha</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px 80px' }}>
                  ...
                  {photos.map((p, i) => {
                    ...
                    return (
                      <div key={p.id}
                        ...
                        {p.kind === 'new' && (
                          <div style={{ position: 'absolute', bottom: 4, left: 4, background: 'rgba(77,184,112,0.9)', borderRadius: 2, padding: '1px 5px', fontSize: 9, fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            Nueva
                          </div>
                        )}
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Link to={`/admin/ficha-cliente/${clienteId}/ficha/${fichaId}`}
              style={{ display: 'flex', alignItems: 'center', padding: '13px 24px', background: '#f5f7fa', border: '1px solid #dce4ec', borderRadius: 2, fontSize: 15, color: '#7a8fa6', textDecoration: 'none', fontWeight: 500 }}>
              Cancelar
            </Link>
          </div>
        </div>
