...
serve(async (req) => {
  try {
    ...
    const { nombre, email, telefono, mensaje } = record
    ...
    const html = `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; color: #1a1a1a;">
        <div style="border-left: 4px solid #3DAA6E; padding-left: 16px; margin-bottom: 24px;">
          <h1 style="font-size: 22px; font-weight: 300; margin: 0; color: #0F2535;">Nuevo mensaje de contacto</h1>
          <p style="margin: 4px 0 0; font-size: 13px; color: #7a8a96;">SDM Capital — Panel de contacto</p>
        </div>

        <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #e8edf2; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #7a8a96; width: 120px;">Nombre</td>
        ...
        <a href="mailto:${email}?subject=Re: Consulta SDM Capital&body=Hola ${nombre},"
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to:   TO_EMAIL,
        subject: `Nuevo mensaje de ${nombre} — SDM Capital`,
        html,
      }),
    })
